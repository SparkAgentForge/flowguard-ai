...
class VideoAudit(Base):
    ...
    execution_trace: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)
    review_requests: Mapped[list[dict[str, Any]]] = mapped_column(JSON, default=list)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now)
...
class AuditFinding(Base):
    ...
    evidence_status: Mapped[str] = mapped_column(String(40), default="CONFIRMED")
    evidence_score: Mapped[int] = mapped_column(Integer, default=0)
    occluded: Mapped[bool] = mapped_column(Boolean, default=False)
    chunk_idx: Mapped[int | None] = mapped_column(Integer, nullable=True)
